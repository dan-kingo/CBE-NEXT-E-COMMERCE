...
import CategoryListSkeleton from "~/features/category/components/CategoryListSkeleton.vue";
...
const allCategoryOptions = computed(() => {
    ...
    if (isEditMode.value && editingCategoryId.value) {
        return options.filter(
            (item) => item.category.id !== editingCategoryId.value,
        );
    }
...
const pageSummary = computed(() => {
    ...
    const start =
        categoriesPagination.value.page * categoriesPagination.value.size + 1;
...
const fetchCategories = async (
    ...
    const size =
        categoriesPagination.value.size > 1
...
const toggleActionMenu = (categoryId: string) => {
    openActionMenuForId.value =
        openActionMenuForId.value === categoryId ? null : categoryId;
};
...
onMounted(async () => {
    ...
    const initialPage = categoriesLoaded.value
        ? categoriesPagination.value.page
        : 0;
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <div class="flex items-center gap-2 bg-muted/30 rounded-lg px-3 py-2">
                                <!-- <select class="bg-transparent outline-none text-sm text-muted-foreground">
                                    <option>All</option>
                                </select> -->
                                <Input v-model="searchQuery" placeholder="Search by name or slug"
                                    class="max-w-sm bg-transparent" />
                            </div>

                            <Button class="cursor-pointer bg-brand text-white hover:bg-brand-hover rounded-lg px-4 py-2"
                ...
                <div v-else class="pb-10">
                            ...
                            <div class="flex justify-end">
                                <div class="relative" data-action-menu>
                                    ...
                                    <div v-if="openActionMenuForId === row.category.id"
                                        class="absolute left-0 bottom-full z-80 mb-2 min-w-40 rounded-md border bg-background p-1 shadow-lg">
                                        <button
                        ...
                        <table
                            ...
                            <thead class="bg-muted/20">
                                <tr class="text-left">
                                    ...
                                    <th class="py-3 px-4 text-sm text-muted-foreground">Slug</th>
                                    <th class="py-3 px-4 text-sm text-muted-foreground">Children</th>
                                    <th class="py-3 px-4 text-sm text-muted-foreground">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(row, index) in visibleCategoryRows" :key="row.category.id"
                                    class="border-b align-top hover:bg-muted/10 transition-colors">
                                    <td class="py-4 px-4 font-medium">
                                        <div :style="{ paddingLeft: `${row.depth * 16}px` }"
                                            ...
                                            <button class="text-brand hover:underline text-base font-medium text-left"
                                                @click="goToEditCategory(row.category)">{{ row.category.name }}</button>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4 text-muted-foreground">{{ row.category.slug }}</td>
                                    <td class="py-4 px-4">{{ getChildrenCount(row.category) }}</td>
                                    <td class="py-4 px-4">
                                        <div class="relative" data-action-menu>
                                            <Button class="cursor-pointer" size="icon-sm" variant="ghost"
                                                @click="toggleActionMenu(row.category.id)">
                                            ...
                                            <div v-if="openActionMenuForId === row.category.id" :class="index === visibleCategoryRows.length - 1
                                                ? 'absolute right-full bottom-full z-80 mb-2 min-w-40 rounded-md border bg-background p-1 shadow-lg'
                                                : 'absolute right-full top-6 z-80 mb-2 min-w-40 rounded-md border bg-background p-1 shadow-lg'
                                                ">
                                                <button
                                                    class="w-full rounded-sm px-2 py-1.5 text-left text-sm hover:bg-muted cursor-pointer"
                                ...
                                <tr v-if="!visibleCategoryRows.length">
                                    <td colspan="4" class="py-6 text-center text-muted-foreground">No categories found.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="relative z-10 mt-4 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div class="flex items-center gap-2">
                            <Button class="cursor-pointer" size="sm" variant="outline" :disabled="!categoriesPagination.hasPrevious || isCategoriesLoading
