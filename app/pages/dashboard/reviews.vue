...
const moderationStatusOptions: Array<{ label: string; value: ReviewModerationStatus | "" }> = [
    { label: "All", value: "" },
...
const visibilityStatusOptions: Array<{ label: string; value: ReviewVisibilityStatus | "" }> = [
    { label: "All", value: "" },
...
const isInitialLoading = computed(() => isReviewsLoading.value && !reviewsLoaded.value);
...
const averageRatingLabel = computed(() => {
    const value = Number(reviewsSummary.value.averageRating || 0);
...
const loadTenantOptions = async () => {
    try {
        await adminDataStore.ensureTenants({ page: 0, size: 20 });
    } catch {
        // Tenant options are non-blocking; review list can still load.
    }
};

const loadReviews = async (page = reviewsPagination.value.page, force = false) => {
    const size = reviewsPagination.value.size > 1
...
const runDecision = async (
    ...
    try {
        ...
        void adminDataStore.revalidateReviews({
            ...
            size: reviewsPagination.value.size > 1
                ? reviewsPagination.value.size
                : DEFAULT_PAGE_SIZE,
        });
    } catch (error) {
    ...
    <section class="space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <Card class="px-6 py-4">
                ...
                <p class="mt-1 text-2xl font-semibold">{{ averageRatingLabel }}</p>
            </Card>
            <Card class="px-6 py-4">
                <p class="text-xs text-muted-foreground">Published Reviews</p>
                <p class="mt-1 text-2xl font-semibold">{{ reviewsSummary.totalPublishedReviews }}</p>
            </Card>
            <Card class="px-6 py-4">
                <p class="text-xs text-muted-foreground">Total Loaded</p>
                <p class="mt-1 text-2xl font-semibold">{{ reviewsPagination.totalElements }}</p>
            </Card>
        </div>
        ...
        <Card class="px-6 py-4">
            <div class="grid gap-3 md:grid-cols-5">
                <div class="space-y-1">
                    <Label for="review-tenant">Tenant</Label>
                    <select id="review-tenant" v-model="filters.tenantId"
        ...
        <Card class="px-6">
            <div class="space-y-4">
                ...
                <div v-if="isInitialLoading" class="text-sm text-muted-foreground">Loading reviews...</div>
                        ...
                        <tbody>
                            <tr v-for="review in reviews" :key="review.id" class="border-b align-top">
                                <td class="py-2">
                                    ...
                                    <p class="text-xs text-muted-foreground">{{ review.authorName || `User
                                        #${review.userId}` }}</p>
                                    <p class="mt-1 line-clamp-2 text-xs text-muted-foreground">{{ review.comment }}</p>
                                ...
                                <td class="py-2">{{ new Date(review.createdAt).toLocaleString() }}</td>
                                <td class="py-2">
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        <Button class="cursor-pointer" size="sm" variant="destructive"
                                            :disabled="decidingById[review.id]" @click="openRejectDialog(review)">
                                            Reject
                                        </Button>
                                    </div>
                                </td>
                            ...
                            <tr v-if="!reviews.length">
                                <td colspan="6" class="py-4 text-center text-muted-foreground">No reviews found.</td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <div v-if="isRejectDialogOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
            ...
            <div role="dialog" aria-modal="true" aria-labelledby="reject-review-title"
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <Button class="cursor-pointer" variant="destructive" @click="submitRejectDecision">Reject
                        review</Button>
                </div>
            </div>
        </div>
